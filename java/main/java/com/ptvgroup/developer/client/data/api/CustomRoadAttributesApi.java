...
public class CustomRoadAttributesApi {
   ...
   * @param points A point, a polyline or a polygon to select roads.  For a single point the road closest to this point will be returned. Several points where the first and last  point are different will be considered a polyline and all roads intersected by this polyline will be  returned. If the polyline is closed, i.e. the first and the last point are the same, the polyline defines  a polygon and all roads whose complete polyline is within the polygon will be returned. A polygon must be  valid according to [OGC](https://www.ogc.org/standard/sfa/). Holes within polygons are not supported. Ferries will not be selected.  Format: &#x60;&lt;point1_lat&gt;,&lt;point1_lon&gt;,...,&lt;pointN_lat&gt;,&lt;pointN_lon&gt;&#x60;.  A request will be rejected if it * does not contain an even number of coordinates, * contains a closed polyline defining an invalid polygon according to OGC, * contains invalid coordinates or * covers more than 5000 roads. (required)
   * @param polylineFormat  (optional, default to GEO_JSON)
